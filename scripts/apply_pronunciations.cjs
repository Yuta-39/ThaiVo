...
const repairedPath = path.join(__dirname, 'repaired_pronunciations.json');
...
try {
  ...
  console.log(`🔍 ${repairedItems.length} 件の発音修正データを適用します...`);

  // マップ作成
  const repairedMap = new Map();
  for (const item of repairedItems) {
    repairedMap.set(item.id, item.pronunciation);
  }
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (repairedMap.has(orig.id)) {
      const origPron = orig.pronunciation;
      const fixedPron = repairedMap.get(orig.id);
      if (origPron !== fixedPron) {
        orig.pronunciation = fixedPron;
        applyCount++;
      }
    }
  }

  console.log(`📊 適用された発音修正項目数: ${applyCount} 件`);
...
export const vocabulary: VocabularyItem[] = `;
    ...
    console.log('✅ 発音修正の適用が完了し、ファイルに保存されました。');
  } else {
    console.log('⚠️ 適用された発音修正項目はありませんでした。');
  }
